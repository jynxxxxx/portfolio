export default function IconCarousel() {
  return (
    <div className="iconctn">
      <div className="icontrack">
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original-wordmark.svg" alt="html5" width="90" height="90" />
          <div className="iconname">HTML5</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="javascript" width="90" height="90" />
          <div className="iconname">Javascript</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original-wordmark.svg" alt="css3" width="90" height="90" />
          <div className="iconname">CSS</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg" alt="nodejs" width="90" height="90" />
          <div className="iconname">NodeJS</div>
        </div>
        <div className="icon">
          <img src="https://cdn.worldvectorlogo.com/logos/nextjs-2.svg" alt="nextjs" width="90" height="90" />
          <div className="iconname">Next.js</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg" alt="react" width="90" height="90" />
          <div className="iconname">React</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg" alt="tailwind" width="90" height="90" />
          <div className="iconname">Tailwind</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" alt="typescript" width="90" height="90" />
          <div className="iconname">Typescript</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/d00d0969292a6569d45b06d3f350f463a0107b0d/icons/webpack/webpack-original-wordmark.svg" alt="webpack" width="90" height="90" />
          <div className="iconname">Webpack</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original-wordmark.svg" alt="express" width="90" height="90" />
          <div className="iconname">Express</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/babeljs/babeljs-icon.svg" alt="babel" width="90" height="90" />
          <div className="iconname">Babel</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/jestjsio/jestjsio-icon.svg" alt="jest" width="90" height="90" />
          <div className="iconname">Jest</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" alt="mysql" width="90" height="90" />
          <div className="iconname">MySQL</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/getpostman/getpostman-icon.svg" alt="postman" width="90" height="90" />
          <div className="iconname">Postman</div>
        </div>
      </div>
      <div className="icontrack">
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original-wordmark.svg" alt="html5" width="90" height="90" />
          <div className="iconname">HTML5</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="javascript" width="90" height="90" />
          <div className="iconname">Javascript</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original-wordmark.svg" alt="css3" width="90" height="90" />
          <div className="iconname">CSS</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg" alt="nodejs" width="90" height="90" />
          <div className="iconname">NodeJS</div>
        </div>
        <div className="icon">
          <img src="https://cdn.worldvectorlogo.com/logos/nextjs-2.svg" alt="nextjs" width="90" height="90" />
          <div className="iconname">Next.js</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg" alt="react" width="90" height="90" />
          <div className="iconname">React</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg" alt="tailwind" width="90" height="90" />
          <div className="iconname">Tailwind</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" alt="typescript" width="90" height="90" />
          <div className="iconname">Typescript</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/d00d0969292a6569d45b06d3f350f463a0107b0d/icons/webpack/webpack-original-wordmark.svg" alt="webpack" width="90" height="90" />
          <div className="iconname">Webpack</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original-wordmark.svg" alt="express" width="90" height="90" />
          <div className="iconname">Express</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/babeljs/babeljs-icon.svg" alt="babel" width="90" height="90" />
          <div className="iconname">Babel</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/jestjsio/jestjsio-icon.svg" alt="jest" width="90" height="90" />
          <div className="iconname">Jest</div>
        </div>
        <div className="icon">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" alt="mysql" width="90" height="90" />
          <div className="iconname">MySQL</div>
        </div>
        <div className="icon">
          <img src="https://www.vectorlogo.zone/logos/getpostman/getpostman-icon.svg" alt="postman" width="90" height="90" />
          <div className="iconname">Postman</div>
        </div>
      </div>
    </div>
  )
}